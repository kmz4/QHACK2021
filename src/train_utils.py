...
def train_circuit(circuit, parameter_shape, X_train, Y_train, batch_size, learning_rate, **kwargs):
    ...
    if kwargs['readout_layer'] == 'one_hot':
        var = np.ones(parameter_shape) * 0.1
    ...
    opt = optim(
        stepsize=learning_rate)  # all optimizers in autograd module take in argument stepsize, so this works for all
    start = time.time()
    for _ in range(kwargs['nsteps']):
        ...
        if kwargs['readout_layer'] == 'one_hot':
            # grad = opt.compute_grad(lambda v: ohe_cost_fcn(v, circuit, X_train_batch, Y_train_batch), (var, ), kwargs={})
            # print(grad)
    ...
    if kwargs['rate_type'] == 'accuracy':
        ...
        if kwargs['readout_layer'] == 'one_hot':
            ...
            w = var[-n:]
            theta = var[:-n]
            prediction = [int(np.round(2. * (1.0 / (1.0 + exp(np.dot(-w, circuit(theta, features=x))))) - 1., 0)) for x
                          in X_validation_batch]
        end = time.time()
        inftime = (end - start) / len(X_validation_batch)
        err_rate = (1.0 - accuracy(predictions,
                                   Y_validation_batch)) + 10 ** -7  # add small epsilon to prevent divide by 0 errors
